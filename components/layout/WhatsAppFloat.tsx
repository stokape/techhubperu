...
export function WhatsAppFloat() {
  const message = encodeURIComponent("Hola TechHub, quisiera más información");
  const href = `https://wa.me/${siteConfig.contact.whatsapp}?text=${message}`;

  return (
    <a
      data-el="whatsapp-float.button"
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Escríbenos por WhatsApp"
      className="fixed right-5 bottom-5 z-[70] flex h-[50px] w-[50px] items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_8px_20px_-6px_rgba(0,0,0,.4)] transition-transform hover:-translate-y-0.5 hover:scale-[1.04] max-[600px]:right-3.5 max-[600px]:bottom-3.5 max-[600px]:h-[46px] max-[600px]:w-[46px]"
    >
